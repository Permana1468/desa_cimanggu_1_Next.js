...
async function main() {
  ...
  const user = await prisma.user.upsert({
    where: { email: 'operator_fix@cimanggu1.desa.id' },
    update: {
      fullName: 'Operator Desa (Fix)',
      role: 'ADMIN_DESA', // Role paling stabil
      passwordHash,
      isActive: true,
      tenantId
    },
    create: {
      email: 'operator_fix@cimanggu1.desa.id',
      fullName: 'Operator Desa (Fix)',
      role: 'ADMIN_DESA',
      passwordHash,
      isActive: true,
  ...
  console.log(`✅ Stable Operator account created: ${user.email}`);
}
