        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-8">
            <div className="space-y-4">
                <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/5 border border-white/10 text-slate-400 text-[10px] font-bold uppercase tracking-widest">
                    <LayoutGrid size={14} className="text-emerald-500" /> Panel Operasional Desa Cimanggu I
                </div>
                <h1 className="text-4xl md:text-5xl font-black text-white tracking-tight">
                    Dashboard Desa
                </h1>
                <p className="text-slate-400 max-w-xl leading-relaxed">
                    Selamat pagi, <span className="text-white font-bold">{session?.user?.name}</span>. Mari kelola layanan persuratan dan data warga desa kita hari ini.
                </p>
                ...
                <div className="flex flex-wrap gap-4 pt-4">
                    ...
                    <div className="flex items-center gap-2 text-slate-300 bg-white/5 px-4 py-2 rounded-xl border border-white/5 text-xs font-medium">
                        <Sparkles size={14} className="text-emerald-400" /> {stats.suratPending} Surat menunggu diproses
                    </div>
                    <div className="flex items-center gap-2 text-slate-300 bg-white/5 px-4 py-2 rounded-xl border border-white/5 text-xs font-medium">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:flex gap-4">
                ...
                <QuickStat label="LAPORAN TERVERIFIKASI" value={stats.laporanVerified.toString()} />
            ...
            <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-200/60">
                <div className="flex items-center justify-between mb-8">
                    <div>
                        ...
                        <p className="text-slate-500 text-sm">Kelola pengajuan surat dari warga dan teruskan ke Sekdes.</p>
                    </div>
                    <button className="bg-slate-50 px-4 py-2 rounded-xl border border-slate-100 flex items-center gap-2 text-slate-600 text-xs font-bold hover:bg-slate-100 transition-all">
        ...
        <div className="space-y-6">
            <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-200/60">
                ...
                <div className="grid grid-cols-1 gap-4">
                    <ActionButton 
                        icon={UserPlus} 
                        title="Daftarkan RT/RW" 
                        desc="Buat akun akses perangkat wilayah" 
                        color="bg-blue-500" 
                    />
                    <ActionButton 
                        icon={Search} 
                        title="Cari Data Warga" 
                        desc="Validasi NIK untuk persuratan" 
                        color="bg-emerald-500" 
                    />
                    <ActionButton 
                        icon={Sparkles} 
                        title="Update Berita" 
                        desc="Kelola konten landing page" 
                        color="bg-amber-500" 
                    />
                </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-blue-600 to-blue-700 rounded-[2.5rem] p-8 text-white shadow-xl relative overflow-hidden group">
                ...
                <h3 className="text-lg font-black mb-2 relative z-10">Bantuan Sistem</h3>
                <p className="text-blue-100 text-xs leading-relaxed mb-6 relative z-10">Mengalami kendala dalam verifikasi surat? Hubungi tim IT pusat kendali desa.</p>
