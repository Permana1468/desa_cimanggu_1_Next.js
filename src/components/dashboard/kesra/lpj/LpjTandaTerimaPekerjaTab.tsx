...
import { CheckCircle2, Search, Plus, Printer, ChevronLeft, ChevronRight, Edit3, Trash2, FileText } from "lucide-react";
...
export function LpjTandaTerimaPekerjaTab({ session }: { session: any }) {
  ...
  const [data, setData] = useState<any[]>([
    {
      id: "1",
      noKwitansi: "KW/KESRA/2026/001",
      namaPekerja: "Ahmad Subagja",
      jabatan: "Tukang Utama",
      kegiatan: "Pembangunan TTP Dusun I",
      nominalBayar: 1680000,
      tanggalBayar: "2026-07-16",
      statusTTD: "TERTANDA TANGAN"
    },
    {
      id: "2",
      noKwitansi: "KW/KESRA/2026/002",
      namaPekerja: "Ujang Iskandar",
      jabatan: "Pekerja / Laden",
      kegiatan: "Pembangunan TTP Dusun I",
      nominalBayar: 1080000,
      tanggalBayar: "2026-07-16",
      statusTTD: "TERTANDA TANGAN"
    }
  ]);
  ...
  const filteredData = useMemo(() => {
    return data.filter(item => 
      ...
      item.noKwitansi.toLowerCase().includes(search.toLowerCase()) ||
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="text-xl font-bold text-slate-800">4. Tanda Terima Pekerja</h1>
              <p className="text-xs text-slate-500">Kwitansi & Tanda Terima Honor/Upah Pekerja Pembangunan LPJ Kesra</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <button 
            onClick={() => alert("Fitur Tanda Terima Pekerja Siap Disesuaikan")}
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 space-y-4">
        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="relative w-full md:w-80">
            ...
            <input
              type="text"
              placeholder="Cari penerima / kwitansi / kegiatan..."
              value={search}
          ...
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-600 text-xs uppercase font-bold">
              <tr>
                ...
                <th className="px-6 py-4">Nama Penerima / Pekerja</th>
                <th className="px-6 py-4">Kegiatan</th>
                <th className="px-6 py-4">Nominal Upah</th>
                <th className="px-6 py-4">Status TTD</th>
              ...
              {paginatedData.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-6 py-12 text-center text-slate-400">
                    ...
                    <p className="font-semibold text-slate-600">Belum ada data tanda terima pekerja.</p>
                  </td>
                </tr>
              ) : (
                paginatedData.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-6 py-4 font-mono font-bold text-rose-700">{item.noKwitansi}</td>
                    <td className="px-6 py-4 font-bold text-slate-800">
                      <div>{item.namaPekerja}</div>
                      <div className="text-xs font-normal text-slate-500">{item.jabatan}</div>
                    </td>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4 border-t border-slate-100 text-xs">
          <div className="text-slate-500 font-bold">
            {totalItems === 0
              ...
              : itemsPerPage === "ALL"
              ? `Menampilkan seluruh ${totalItems} tanda terima.`
