...
import { ShoppingBag, Search, Plus, Printer, ChevronLeft, ChevronRight, Edit3, Trash2 } from "lucide-react";
...
export function LpjPesananBarangTab({ session }: { session: any }) {
  ...
  const [data, setData] = useState<any[]>([
    {
      id: "1",
      noPesanan: "PO/KESRA/2026/001",
      namaBarang: "Semen Gersik (50 kg)",
      supplier: "TB. Cimanggu Jaya",
      jumlah: "120 Sak",
      hargaSatuan: 72000,
      totalHarga: 8640000,
      tanggalPesanan: "2026-07-10",
      status: "DITERIMA"
    },
    {
      id: "2",
      noPesanan: "PO/KESRA/2026/002",
      namaBarang: "Besi Beton 10mm SNI",
      supplier: "TB. Maju Bersama",
      jumlah: "50 Batang",
      hargaSatuan: 95000,
      totalHarga: 4750000,
      tanggalPesanan: "2026-07-15",
      status: "PROSES"
    }
  ]);
  ...
  const filteredData = useMemo(() => {
    return data.filter(item => 
      ...
      item.noPesanan.toLowerCase().includes(search.toLowerCase()) ||
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="text-xl font-bold text-slate-800">2. Pesanan Barang</h1>
              <p className="text-xs text-slate-500">Formulir Pesanan & Nota Pembelian Material Pembangunan LPJ Kesra</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <button 
            onClick={() => alert("Fitur Pesanan Barang Siap Disesuaikan")}
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 space-y-4">
        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="relative w-full md:w-80">
            ...
            <input
              type="text"
              placeholder="Cari pesanan / nama barang / supplier..."
              value={search}
          ...
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-50 text-slate-600 text-xs uppercase font-bold">
              <tr>
                ...
                <th className="px-6 py-4">Nama Barang & Volume</th>
                <th className="px-6 py-4">Tokoh / Supplier</th>
              ...
              {paginatedData.length === 0 ? (
                ...
                paginatedData.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-6 py-4 font-mono font-bold text-rose-700">{item.noPesanan}</td>
                    <td className="px-6 py-4 font-bold text-slate-800">
                      <div>{item.namaBarang}</div>
                      <div className="text-xs font-normal text-slate-500">Jumlah: {item.jumlah}</div>
                    </td>
                    <td className="px-6 py-4 text-slate-700 font-medium">{item.supplier}</td>
                    <td className="px-6 py-4 text-xs font-bold text-emerald-700">
                      Rp {item.totalHarga.toLocaleString("id-ID")}
                    </td>
                    <td className="px-6 py-4">
                      <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold ${
                        item.status === 'DITERIMA' ? 'bg-emerald-100 text-emerald-800' : 'bg-amber-100 text-amber-800'
                      }`}>
                        {item.status}
                      </span>
                    </td>
