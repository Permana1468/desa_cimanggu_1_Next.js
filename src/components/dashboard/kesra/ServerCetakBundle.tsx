...
import { ServerCetakUsulanUhc } from "./ServerCetakUsulanUhc";
import { ServerCetakSkkm } from "./ServerCetakSkkm";
import { ServerCetakSptjm } from "./ServerCetakSptjm";

export function ServerCetakBundle({ data, sptjmData, onBack }: { data: any, sptjmData: any, onBack?: () => void }) {
  ...
  return (
    <div className="bundle-container bg-slate-100 min-h-screen py-8 text-slate-800 relative z-50">
      ...
      <div className="flex flex-col gap-12 print:gap-0 bundle-content">
        <div>
          <ServerCetakUsulanUhc data={data} isBundle={true} />
        </div>
        <div>
          <ServerCetakSkkm data={data} isBundle={true} />
        </div>
        {sptjmData && (
          <div>
            <ServerCetakSptjm data={sptjmData} isBundle={true} />
          </div>
        )}
      </div>
    </div>
